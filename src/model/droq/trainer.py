...
from model.droq.actor_critic import SquashedGaussianActor, TwinDroQCritic
...
class DroQTrainer:
    def __init__(
        ...
        self.critic_target = TwinDroQCritic(
        ...
        print(
            f"DroQ device={self.device} params={param_count:,} alpha={self.alpha:.4f} min_alpha={self.min_alpha:.4f} "
            f"dropout={self.dropout_p:.4f} layer_norm={self.layer_norm} utd={self.updates_per_step}"
        )
        print(
            ...
            if global_step % log_interval == 0:
                ...
                with open(current_run_folder.get_file_name("data.csv"), "a+") as f:
                    f.write(
                        ...
                        f"{self.dropout_p},{int(self.layer_norm)},{self.updates_per_step}"
                        f"\n"
                    )
                ...
                print(
                    ...
                    f"dropout={self.dropout_p:.4f} ln={self.layer_norm} utd={self.updates_per_step}"
                )
    ...
    def save(self, path: str = "droq_tmrl.pt"):
        torch.save(
            {
    ...
    def load(self, path: str = "droq_tmrl.pt"):
        ...
        self.dropout_p = float(ckpt.get("dropout_p", self.dropout_p))
        self.layer_norm = bool(ckpt.get("layer_norm", self.layer_norm))
        self.updates_per_step = int(ckpt.get("updates_per_step", self.updates_per_step))
